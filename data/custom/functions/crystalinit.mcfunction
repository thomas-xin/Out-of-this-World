...
execute if entity @s[y=-64,dy=8] run tp @s ~ ~16 ~
execute if entity @s[y=-64,dy=16] run tp @s ~ ~8 ~
execute at @s run tp @s ^16 ^-8 ^16
execute at @s run summon area_effect_cloud ~ ~ ~ {Duration:2,Tags:["random","r1"]}
execute at @s run summon area_effect_cloud ~ ~ ~ {Duration:2,Tags:["random","r2"]}
execute at @s run summon area_effect_cloud ~ ~ ~ {Duration:2,Tags:["random","r3"]}
...
execute at @e[tag=pocketrandomizer] as @e[tag=random,sort=random,limit=1] unless entity @s[tag=r1] run summon armor_stand ~ ~ ~ {Tags:["pocketrandomizer"],Invulnerable:1}
kill @e[tag=random]
execute at @s run setblock ~ ~ ~ structure_block[mode=load]{metadata:"stonegen",mirror:"NONE",rotation:"NONE",posX:-6,mode:"LOAD",posY:-5,sizeX:13,posZ:-6,integrity:0.96f,showair:0b,name:"custom:stonegen",sizeY:13,sizeZ:13,showboundingbox:1b}
