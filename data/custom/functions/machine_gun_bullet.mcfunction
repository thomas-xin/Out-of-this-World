...
execute at @s[tag=level1,scores={var3=1}] anchored eyes run summon arrow ^ ^ ^.8 {damage:1,Color:12566527,Duration:5,Tags:["level1","despawn","polar_star_bullet","assign"],Motion:[0.,.1,0.]}
execute at @s[tag=level2,scores={var3=1}] anchored eyes run summon arrow ^ ^ ^.8 {damage:2,Color:16777215,Duration:5,Tags:["level2","despawn","polar_star_bullet","assign"],Motion:[0.,.1,0.]}
execute at @s[tag=level3,scores={var3=1}] anchored eyes run summon trident ^ ^ ^.8 {damage:4,Color:12566527,Duration:5,Tags:["level3","despawn","polar_star_bullet","assign"],Motion:[0.,.1,0.]}
...
execute as @s[tag=!thru] as @a[nbt={OnGround:0b,abilities:{flying:0b}},scores={cs_delay=1..},distance=..3,sort=nearest,limit=1] at @s run tp @s ^ ^ ^-.5
