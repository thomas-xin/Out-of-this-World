...
execute if score __fast_loop__ var0 matches 2 as @e[tag=target] at @s run tp @s ~ ~ ~ facing entity @e[tag=priority,distance=..303,sort=random,limit=1] eyes
...
execute if score __global_delay__ var0 matches 0 as @e[tag=wither_main] at @s positioned ~ ~8.1 ~ run function custom:wither_attack
...
execute if score __global_delay__ var0 matches 0 at @e[tag=wither_main] unless block ~ ~-1 ~ air run summon experience_orb ^ ^-.5 ^1 {Value:0,Tags:["bats"]}
...
execute if score __fast_loop__ var0 matches 4 at @e[tag=wither_main] run summon area_effect_cloud ~ ~2 ~ {Duration:4,Tags:["break_box"]}
execute as @e[tag=wither_sprite] run data merge entity @s {Invul:2500}
execute if score __global_delay__ var0 matches 0 run tag @e[tag=found] remove found
execute if score __global_delay__ var0 matches 8 as @e[tag=wither_main] at @e[tag=target_select,sort=random,limit=12] positioned ~-128 ~-8 ~-128 run execute as @e[type=!item,type=!player,type=!trident,type=!wither_skull,type=!wither,type=!experience_orb,type=!area_effect_cloud,type=!end_crystal,type=!bat,type=!armor_stand,type=!wither_skeleton,type=!arrow,type=!dragon_fireball,type=!fireball,type=!falling_block,tag=!found,tag=!launch,tag=!target,dx=256,dy=256,dz=256] run tag @s add found

execute unless entity @e[tag=found,limit=1] at @e[tag=target] run kill @e[tag=target,distance=0.1..2,limit=1]
execute if score __fast_loop__ var0 matches 6 at @e[tag=target] run effect give @e[distance=..3,type=!item,type=!experience_orb,type=!area_effect_cloud,type=!bat] wither 1 0
...
execute if score __slow_loop__ var0 matches 12 as @e[tag=wither_main] at @e[tag=check2,sort=random,limit=7] unless entity @e[tag=table,distance=..80,limit=1] run fill ~-15.5 ~-11.5 ~-15.5 ~15.5 ~11.5 ~15.5 command_block[facing=up]{Command:"summon silverfish ~ ~ ~ {Tags:['tableset','table'],NoAI:1b,Attributes:[{Name:generic.maxHealth,Base:3}],Silent:1b,ActiveEffects:[{Id:6b,Duration:999999,ShowParticles:0b},{Id:14b,Duration:999999,ShowParticles:0b}]}",auto:1} replace crafting_table
execute if score __slow_loop__ var0 matches 14 at @e[tag=tableset] run setblock ~ ~ ~ crafting_table
execute if score __slow_loop__ var0 matches 14 as @e[tag=tableset] run tag @s remove tableset
execute if score __global_delay__ var0 matches 36 as @e run function custom:target_priority