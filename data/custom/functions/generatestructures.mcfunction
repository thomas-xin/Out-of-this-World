...
kill @e[tag=crystal]
execute at @e[tag=emeraldgen] run setblock ~ ~ ~ structure_block[mode=load]{metadata:"emeraldgen",mirror:"NONE",rotation:"NONE",posX:-6,mode:"LOAD",posY:-5,sizeX:13,posZ:-6,integrity:0.11f,showair:0b,name:"custom:emeraldgen",sizeY:13,sizeZ:13,showboundingbox:1b}
execute at @e[tag=emeraldgen] run setblock ~ ~-1 ~ redstone_block
execute at @e[tag=emeraldgen] run setblock ~ ~ ~ structure_block[mode=load]{metadata:"diamondgen",mirror:"NONE",rotation:"NONE",posX:-6,mode:"LOAD",posY:-5,sizeX:13,posZ:-6,integrity:0.07f,showair:0b,name:"custom:diamondgen",sizeY:13,sizeZ:13,showboundingbox:1b} destroy
...
execute as @e[tag=s_cave_crystal_generator] run function custom:crystalinit
kill @e[tag=pocketrandomizer]
kill @e[tag=s_cave_crystal_generator]
tag @e[tag=s_cave_crystal,tag=!s_done] add s_done

execute at @a as @e[type=zombie,tag=!crystal,nbt={IsBaby:1b},distance=64..,sort=random,limit=1] at @s if entity @s[y=0,dy=32] if block ~ ~-2 ~ stone unless entity @e[tag=s_cave_spawner,distance=..513] run summon armor_stand ~ ~ ~ {Marker:1,Invisible:1,Invulnerable:1,Tags:["s_cave_spawner"]}
...
kill @e[tag=s_reset]
execute at @a unless entity @e[tag=s_nether_labyrinth,distance=..320] at @e[type=magma_cube,nbt=!{Size:0},distance=64..,sort=random,limit=1] if block ~ 31 ~ lava[level=0] if block ~ 31 ~16 lava if block ~16 31 ~ lava if block ~ 31 ~-16 lava if block ~-16 31 ~ lava unless entity @e[tag=s_nether_labyrinth,distance=..385] run summon armor_stand ~ ~ ~ {Marker:1,Invisible:1,Invulnerable:1,Tags:["s_nether_labyrinth"]}
