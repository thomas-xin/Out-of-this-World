...
kill @e[tag=s_reset]
execute at @a at @e[type=magma_cube,nbt=!{Size:0},distance=64..,sort=random,limit=1] if block ~ 31 ~ lava[level=0] if block ~ 31 ~16 lava if block ~16 31 ~ lava if block ~ 31 ~-16 lava if block ~-16 31 ~ lava unless entity @e[tag=s_nether_labyrinth,distance=..750] run summon armor_stand ~ ~ ~ {Marker:1,Invisible:1,Invulnerable:1,Tags:["s_nether_labyrinth"]}
