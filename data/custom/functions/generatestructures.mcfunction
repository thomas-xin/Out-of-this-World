...
# tag @e[tag=s_nether_crystal,tag=!s_done] add s_done
...
kill @e[tag=s_reset]
execute as @e[type=magma_cube,tag=!gen_ticked,nbt=!{Size:0},sort=random,limit=1] at @s unless entity @p[distance=..64] if block ~ 31 ~ lava[level=0] if block ~ 31 ~16 lava if block ~16 31 ~ lava if block ~ 31 ~-16 lava if block ~-16 31 ~ lava unless entity @e[tag=s_nether_labyrinth,distance=..750] run summon armor_stand ~ ~ ~ {Marker:1,Invisible:1,Invulnerable:1,Tags:["s_nether_labyrinth"]}
